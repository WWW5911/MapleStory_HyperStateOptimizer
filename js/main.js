...
class CharStatus {
    ...
    defense_c() {
        let ret = 1 - this.defense * ( 1 - this.ignore) // (1-0.86) * (1-ignore)
        if (ret < 0) return 0
        return ret
    }
    realAtk() {
...
function calculate(){
    ...
    att_p = parseFloat( document.getElementById("att_p").value ) / 100;
    ...
    ctri_a = parseFloat( document.getElementById("ctri_a").value ) / 100;
    ...
    if(isNaN(mainV+ subV+ att+ att_p+ tot_a+ boss_a+ base_ig+ ctri_a + defense)){
        if(isNaN(mainV)) 
