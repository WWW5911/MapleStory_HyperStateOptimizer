class CharStatusDS {
    ...
    base() {
        return (4 * this.mainV + this.subV + this.subV2) * this.weapon * (this.att * (1 + this.att_p) ) * 0.01
    }
    defense_c() {
    ...
    clone() {
        return new CharStatusDS(this.mainV, this.subV, this.subV2, this.weapon, this.att, this.att_p, this.boss_a, this.defense, this.base_ig, this.ctri_a,
...
function dfsDS(charStatus, points){
...
function optim_GreddyDS(charStatus, points){
    last_point = -1
 ...
 */
// my = new CharStatusDS( 49760, 6475,  1.2,    3318, 0.97, 3.93+0.57, 3.8,    0.9658, 0.864 )


// best_status = optim_GreddyDS(my, 1287)
...
function exampleDS(){
    ...
    document.getElementById("subV2").value = 3472;
...
function calculateDS(){
    ...
    if(isNaN(mainV+ subV + subV2 + att+ att_p+ tot_a+ boss_a+ base_ig+ ctri_a + defense) || att <= 0 || mainV <= 0 || subV<= 0 || subV2 <= 0 ){
        ...
        allBlack();
        document.getElementById("output").innerHTML = "計算中>< <br> 目前推演至：" ;
        my = new CharStatusDS( mainV, subV, subV2, 1, att, att_p, boss_a+tot_a, defense, base_ig, ctri_a )
        best_status = optim_GreddyDS(my, points)
        ...
        document.getElementById("output").innerHTML += ' 等<br><br>Boss傷害應升至：' + best_status[0].p_Bharm + " 等<br><br>總傷害　應升至：" + best_status[0].p_harm
